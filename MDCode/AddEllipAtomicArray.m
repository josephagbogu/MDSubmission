function AddEllipAtomicArray(rad1, rad2, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
W = (2*rad1 - 1) * AtomSpacing;

xp(1, :) = linspace(-L/2, L/2, 2*rad1);
yp(1, :) = linspace(-W/2, W/2, 2*rad1);

numAtoms = 0;
for i = 1:2*rad1
    for j = 1:2*rad1
        if (xp(i)^2)/(rad1^2) + (yp(j)^2)/(rad2^2) <= (AtomSpacing)^2
            numAtoms = numAtoms+1;
